...
public class Comment extends AbstractDomainObject implements Identifiable {
    ...
    @ForeignKey(name = "FK_COMMENT_MOVIE")
    @OnDelete(action = OnDeleteAction.CASCADE)
    @NotNull
    private Movie movie;
    @Transient
    private transient CommentKey cachedCommentKey;
    ...
    public CommentKey getCommentKey() {
        if (cachedCommentKey == null) {
            cachedCommentKey = new CommentKey(getId(), getUser(), getMovie());
        }
        return cachedCommentKey;
    }
    ...
    public static class CommentKey {
        ...
        public CommentKey(Long id, User user, Movie movie) {
            this.id = id;
            this.user = user;
            this.movie = movie;
